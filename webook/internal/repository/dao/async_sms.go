...
type AsyncSmsDAO interface {
	...
	GetAllResponseTimes(ctx context.Context) ([]int64, error)
}
...
func (g *GORMAsyncSmsDAO) GetAllResponseTimes(ctx context.Context) ([]int64, error) {
	var responseTimes []int64
	err := g.db.WithContext(ctx).Model(&AsyncSms{}).
		Select("utime - ctime").
		Where("status = ?", asyncStatusSuccess).
		Scan(&responseTimes).Error
